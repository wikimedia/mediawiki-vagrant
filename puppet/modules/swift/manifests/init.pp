...
class swift (
    ...
    exec { 'swift-init':
        ...
        user    => 'root',
        unless  => "swift -A http://127.0.0.1:${port}/auth/v1.0 -U testproj:testuser -K testpwd stat -v | grep -Pq 'Auth Token'",
        require => [
            File[$storage_dir],
            File["${storage_dir}/1"],
            File[$cfg_file],
    ...
    exec { 'swift-create-public-container':
        command => "swift -A http://127.0.0.1:${port}/auth/v1.0 -U testproj:testuser -K testpwd upload wiki-local-public /tmp/foo",
        user    => 'root',
        unless  => "curl -s -o /dev/null -w \"%{http_code}\" http://127.0.0.1:${port}/v1/AUTH_testproj/wiki-local-public/tmp/foo | grep -Pq '200'",
        require => [
            Exec['swift-init'],
            File['/tmp/foo'],
        ],
    ...
    exec { 'swift-make-container-public':
        command     => "swift -A http://127.0.0.1:${port}/auth/v1.0 -U testproj:testuser -K testpwd post -r '.r:*' wiki-local-public",
        user        => 'root',
