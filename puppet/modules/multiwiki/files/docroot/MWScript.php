...
EOT
	...
	if (
		!isset( $argv[1] ) ||
		!preg_match( '/^([^-]|--wiki(=|$))/', $argv[1] )
	) {
		$argv = array_merge(
			array( $argv[0], "--wiki=wiki" ),
			array_slice( $argv, 1 )
		);
	} else {
		// Enable MediaWiki-Vagrant's multiwiki processing in LocalSettings.php
		putenv('MULTIWIKI=1');
	}

	require_once( '/srv/multiwiki/MWVersion.php' );
